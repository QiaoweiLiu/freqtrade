...
def factor_rocr(dataframe: pd.DataFrame, pair_name: str) -> pd.DataFrame:
    dataframe[pair_name] = ta.ROCR(dataframe, timeperiod=25)
    return dataframe
...
def factor_adx(dataframe: pd.DataFrame, pair_name: str) -> pd.DataFrame:
    dataframe[pair_name] = ta.ADX(dataframe, timeperiod=27)
    return dataframe
...
def factor_ppo(dataframe: pd.DataFrame, pair_name: str) -> pd.DataFrame:
    dataframe[pair_name] = ta.PPO(dataframe, fastperiod=26, slowperiod=28)
    return dataframe
...
def factor_minus_di(dataframe: pd.DataFrame, pair_name: str) -> pd.DataFrame:
    dataframe[pair_name] = ta.MINUS_DI(dataframe, timeperiod=28)
    return dataframe


def factor_ultosc(dataframe: pd.DataFrame, pair_name: str) -> pd.DataFrame:
    dataframe[pair_name] = ta.ULTOSC(dataframe, timeperiod1=59, timeperiod2=58, timeperiod=58)
    return dataframe
...
def factor_plus_di(dataframe: pd.DataFrame, pair_name: str) -> pd.DataFrame:
    dataframe[pair_name] = ta.PLUS_DI(dataframe, timeperiod=50)
    return dataframe


def factor_plus_dm(dataframe: pd.DataFrame, pair_name: str) -> pd.DataFrame:
    dataframe[pair_name] = ta.PLUS_DM(dataframe, timeperiod=56)
    return dataframe
        ...
        if self.target_symbols.get(target_symbols_key):
            ...
            for key, value in self.pair_data_dict_1h.items():
                ...
                if len(factors_df_dict.keys()) == 0:
                    ...
                    factors_df_dict['MINUS_DI'] = factors_df_dict['MINUS_DI'].join(
                        factor_minus_di(candles, key)[key].to_frame())
                    factors_df_dict['ADXR'] = factors_df_dict['ADXR'].join(factor_adxr(candles, key)[key].to_frame())
                    factors_df_dict['ULTOSC'] = factors_df_dict['ULTOSC'].join(
                        factor_ultosc(candles, key)[key].to_frame())
                    factors_df_dict['MFI'] = factors_df_dict['MFI'].join(factor_mfi(candles, key)[key].to_frame())
                    factors_df_dict['PLUS_DI'] = factors_df_dict['PLUS_DI'].join(
                        factor_plus_di(candles, key)[key].to_frame())
...
def factor_rocr100(dataframe: pd.DataFrame, pair_name: str) -> pd.DataFrame:
    dataframe[pair_name] = ta.ROCR100(dataframe, timeperiod=54)
    return dataframe
...
def factor_dx(dataframe: pd.DataFrame, pair_name: str) -> pd.DataFrame:
    dataframe[pair_name] = ta.DX(dataframe, timeperiod=56)
    return dataframe
...
def factor_ppo(dataframe: pd.DataFrame, pair_name: str) -> pd.DataFrame:
    dataframe[pair_name] = ta.PPO(dataframe, fastperiod=42, slowperiod=34)
    return dataframe
...
def factor_minus_di(dataframe: pd.DataFrame, pair_name: str) -> pd.DataFrame:
    dataframe[pair_name] = ta.MINUS_DI(dataframe, timeperiod=46)
    return dataframe

def factor_ultosc(dataframe: pd.DataFrame, pair_name: str) -> pd.DataFrame:
    dataframe[pair_name] = ta.ULTOSC(dataframe, timeperiod1=57, timeperiod2=58, timeperiod=58)
    return dataframe
...
def factor_mfi(dataframe: pd.DataFrame, pair_name: str) -> pd.DataFrame:
    dataframe[pair_name] = ta.MFI(dataframe, timeperiod=33)
    return dataframe

def factor_plus_di(dataframe: pd.DataFrame, pair_name: str) -> pd.DataFrame:
    dataframe[pair_name] = ta.PLUS_DI(dataframe, timeperiod=47)
    return dataframe
...
def factor_aroondown(dataframe: pd.DataFrame, pair_name: str) -> pd.DataFrame:
    dataframe[pair_name], _ = ta.AROON(dataframe, timeperiod=43)
    return dataframe

def factor_aroonup(dataframe: pd.DataFrame, pair_name: str) -> pd.DataFrame:
    _, dataframe[pair_name] = ta.AROON(dataframe, timeperiod=43)
    return dataframe
        ...
        if self.target_symbols.get(target_symbols_key):
            ...
            for key, value in self.pair_data_dict_1h.items():
                ...
                if len(factors_df_dict.keys()) == 0:
                    ...
                    factors_df_dict['ULTOSC'] = factors_df_dict['ULTOSC'].join(
                        factor_ultosc(candles, key)[key].to_frame())
                    factors_df_dict['ULTOSC1'] = factors_df_dict['ULTOSC1'].join(
                        factor_ultosc1(candles, key)[key].to_frame())
                    factors_df_dict['MFI'] = factors_df_dict['MFI'].join(factor_mfi(candles, key)[key].to_frame())
